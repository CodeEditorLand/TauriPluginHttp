...
impl AbortSender {
	fn abort(self) { let _ = self.0.send(()); }
}
	...
	for (h, v) in headers_raw {
		...
		if is_unsafe_header(&name) {
			#[cfg(debug_assertions)]
			{
				...
				eprintln!(
					"[\x1b[33mWARNING\x1b[0m] if keeping the header is a desired behavior, you \
					 can enable `unsafe-headers` feature flag in your Cargo.toml"
				);
			}
	...
	match scheme {
		"http" | "https" => {
			...
			.is_allowed(&url)
			{
				...
				// In case empty origin is passed, remove it. Some services do not like Origin
				// header so this way we can remove it in explicit way. The default
				// behaviour is still to set it
