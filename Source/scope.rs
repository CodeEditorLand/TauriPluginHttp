...
fn parse_url_pattern(s:&str) -> Result<UrlPattern, urlpattern::quirks::Error> {
	let mut init = urlpattern::UrlPatternInit::parse_constructor_string::<
		regex::Regex,
	>(s, None)?;
...
mod tests {
	...
	fn denied_takes_precedence() {
		...
		assert!(
			!scope
				.is_allowed(&"http://localhost:8080/file.png".parse().unwrap())
		);
		assert!(!scope.is_allowed(
			&"http://localhost:8080?framework=tauri".parse().unwrap()
		));
	}
	...
	fn fixed_url() {
		...
		assert!(scope.is_allowed(&"http://localhost:8080/".parse().unwrap()));
		assert!(
			scope.is_allowed(&"http://localhost:8080/file".parse().unwrap())
		);
		assert!(scope.is_allowed(
			&"http://localhost:8080/path/to/asset.png".parse().unwrap()
		));
		assert!(scope.is_allowed(
			&"http://localhost:8080/path/list?limit=50".parse().unwrap()
		));
	...
	fn fixed_path() {
		...
		assert!(scope.is_allowed(
			&"http://localhost:8080/file.png?q=1".parse().unwrap()
		));
		...
		assert!(!scope.is_allowed(
			&"http://localhost:8080/file.png/other.jpg".parse().unwrap()
		));
	}
	...
	fn pattern_wildcard() {
		...
		assert!(scope.is_allowed(
			&"http://localhost:8080/file.png#head".parse().unwrap()
		));
		assert!(scope.is_allowed(
			&"http://localhost:8080/assets/file.png".parse().unwrap()
		));
		assert!(
			scope.is_allowed(
		...
		assert!(
			!scope.is_allowed(
				&"http://localhost:8080/file.jpeg".parse().unwrap()
			)
		);
	}
	...
	fn domain_wildcard() {
		...
		assert!(
			scope.is_allowed(&"http://something.else#tauri".parse().unwrap())
		);
		assert!(scope.is_allowed(
			&"http://something.else/path/to/file".parse().unwrap()
		));
		assert!(
			scope.is_allowed(
				&"http://something.else?rel=tauri".parse().unwrap()
			)
		);
		assert!(
			scope.is_allowed(
	...
	fn scheme_wildcard() {
		...
		assert!(
			scope.is_allowed(
				&"https://something.else?x=1#frag".parse().unwrap()
			)
		);
	...
	fn validate_query() {
		...
		assert!(
			scope.is_allowed(&"https://tauri.app/path?x=5".parse().unwrap())
		);

		assert!(
			!scope.is_allowed(&"https://tauri.app/path?y=5".parse().unwrap())
		);
	}
	...
	fn validate_hash() {
		...
		assert!(
			scope.is_allowed(&"https://tauri.app/path#frame".parse().unwrap())
		);

		assert!(
			!scope.is_allowed(&"https://tauri.app/path#work".parse().unwrap())
		);
	}
}
